		...
		<block v-for="(item, index) in neworder" :key="index">
			<view class="order-back">
				<view class="order-view">
					<view class="order-img"><image :src="item.logo" mode="aspectFill"></image></view>
					<view class="goods-commodity">{{ item.body }}</view>
					<view class="order-succ" v-if="item.status === 1">付款成功</view>
					<view class="order-succ" v-else>待付款</view>
				</view>
				<!-- 2 -->
				<view>
				...
				<view class="order-Total">
					<view class="order-left"><text v-if="item.status === 0" @click="evaLuate(item.merchantid, item.body)">去评价</text></view>
...
export default {
	...
	// 看订单的前提条件就是是否登录
	onShow() {
		let setdata = uni.getStorageSync('username');
		if (!setdata) {
			let bull = true;
			let tips = '请登录后再查看';
			this.tising(bull, tips);
